...
/*
 * Create our own class inherited from Drag for constrained dragging of the main
 * target window
 */
var TargetDrag = new Class({
	...
	initialize : function() {
		...
		this.handles = (htype == 'array' || htype == 'collection') ? $$(this.options.handle) : $(this.options.handle)
				|| this.element;
		this.mouse = {
			'now' : {},
			'pos' : {}
...
var IIP = new Class(
		{
			/*
			 * Initialize some variables. The constructor takes 2 arguments:
			 * 
			 * i) The id of the main div element in which to create the viewer window
			 * 
			 * ii) A hash containting:
			 ...
			 * image: the full image path (or array of paths) on the server (required)
			 * 
			 * server: the iipsrv server full URL (defaults to
			 ...
			 * 
			 * showNavButtons: whether to show navigation buttons: true (default) or
			 * false
			 * 
			 * scale: pixels per mm
			 */
			...
			loadGrid : function() {
				 ...
				 * Also Center the image if our viewable image is smaller than the
				 * window
				 */
				...
				for (j = starty; j <= endy; j++) {
					for (i = startx; i <= endx; i++) {
						...
						if (this.render == 'spiral') {
							...
							map[ntiles].n = Math.abs(centery - j) * Math.abs(centery - j) + Math.abs(centerx - i)
									* Math.abs(centerx - i);
						}
				...
				for (var m = 0; m < ntiles; m++) {
					...
					var djatoka_x = i * f;
					var djatoka_y = j * f;
					log("djatoka_x=" + djatoka_x + ", djatoka_y=" + djatoka_y);
					if (first) {
						this.top_left_x = (djatoka_x + this.getMultiplier(r, xoffset - 2));
						this.top_left_y = (djatoka_y + this.getMultiplier(r, yoffset - 2));
						...
						for (n = 0; n < this.images.length; n++) {
							tile = new Element('img', {
								...
								'events' : {
									...
									error : function() {
										this.src = this.src + ""; // Try to reload if we have an
										// error
									}
								}
							});
							...
							if (djatoka_x >= 0 && djatoka_y >= 0) {
								...
								src = this.server + "?url_ver=Z39.88-2004&rft_id=" + this.images[n].src + "&svc_id=" + this.svc_id
										+ "&svc_val_fmt=" + this.svc_val_fmt + "&svc.format=image/jpeg&svc.level=" + this.res
										+ "&svc.rotate=0&svc.region=" + djatoka_y + "," + djatoka_x + ",256,256";
								// end djatoka mods
							}

							// We set the source at the end so that the 'load' function is
							// properly fired
							tile.set('src', src);
							tile.injectInside('target');
						}
					} else this.nTilesLoaded++;
				}
			...
			// djatoka mods
			/*
			 * djatoka is based of the offset value of the max resolution, so we need
			...
			setOpenURL : function() {
				var w = this.rgn_w;
				if (this.wid < this.rgn_w) w = this.wid;
				var h = this.rgn_h;
				if (this.hei < this.rgn_h) h = this.hei;
				this.openUrl = this.server + "?url_ver=Z39.88-2004&rft_id=" + this.images[0].src + "&svc_id=" + this.svc_id
						+ "&svc_val_fmt=" + this.svc_val_fmt + "&svc.format=image/jpeg&svc.level=" + this.res
						+ "&svc.rotate=0&svc.region=" + this.top_left_y + "," + this.top_left_x + "," + h + "," + w;
			},
			...
			refresh : function() {
				...
				/*
				 * If no tiles are missing, destroy our refresher timer, fade out our
				 * loading animation and and reset our cursor
				 */
				if (unloaded == 0) {
					$clear(this.refresher);
					this.refresher = null;
			...
			calculateMinSizes : function() {
				...
				if (winWidth > winHeight) {
					...
					else if (ty > 2 * tx) thumb = winHeight / 4;
			...
			createWindows : function() {
				// Get our window size - subtract some pixels to make sure the
				// browser never
				// adds scrollbars
				...
				new Element('a', {
					href : 'http://iipimage.sourceforge.net',
					id : 'logo'
				}).injectInside(this.source);
				new Element('img', {
					src : 'images/djatoka.32x32.png',
					id : 'info',
					styles : {
						opacity : 0.8
					}
				...
				if (Browser.Engine.trident5) {
					$('info')
							.setStyle('filter',
									'progid:DXImageTransform.Microsoft.AlphaImageLoader(enabled=true,src="images/djatoka.32x32.png",sizingMethod=scale)');
				}

				new Tips('#info, #toolbar', {
					className : 'tip', // We need this to force the tip in front of nav
					// window
					// We have to first set opacity to zero to fix a bug in mootools 1.2
					// where the tip appears without any fade in the first time it is
					// loaded
					onShow : function(t) {
			...
			createNavigationWindow : function() {
				...
				// djatoka mods
				// BB: if svc.scale=0,0, then resolver hangs. So don't let this happen!
				if (this.min_x==0) this.min_x=100;
				if (this.min_y==0) this.min_y=100;
				var navimage = new Element('img', {
					id : 'navigation',
					src : this.server + "?url_ver=Z39.88-2004&rft_id=" + this.images[0].src + "&svc_id=" + this.svc_id
							+ "&svc_val_fmt=" + this.svc_val_fmt + "&svc.format=image/jpeg&svc.scale=" + this.min_x + ","
							+ this.min_y
				// end djatoka mods

				// var navimage = new Element( 'img', {
				// id: 'navigation',
				// src: this.server + '?FIF=' + this.images[0].src + '&SDS=' +
				// this.images[0].sds + '&CNT=1.0' +
				...
				// Create our navigation zone and inject inside the navigation
				// div
				...
				var navbuttons = new Element(
						'div',
						{
							id : 'navbuttons',
							html : '<img id="shiftLeft" src="images/left.png"/><img id="shiftUp" src="images/up.png"/><img id="shiftRight" src="images/right.png"/><br/><img id="shiftDown" src="images/down.png"/><br/><img id="zoomIn" src="images/zoomIn.png"/><img id="zoomOut" src="images/zoomOut.png"/><img id="reset" src="images/reset.png"/>&nbsp;&nbsp;<img id="snapshot" width="24" height="24" src="images/djatoka.32x32.png"/>'
						});
				...
				$('snapshot').addEvent('click', function() {
					this.openOpenURL();
				}.bind(this));
				$('shiftLeft').addEvent('click', function() {
					this.scrollTo(-this.rgn_w / 3, 0);
				}.bind(this));
				$('shiftUp').addEvent('click', function() {
					this.scrollTo(0, -this.rgn_h / 3);
				}.bind(this));
				$('shiftDown').addEvent('click', function() {
					this.scrollTo(0, this.rgn_h / 3);
				}.bind(this));
				$('shiftRight').addEvent('click', function() {
					this.scrollTo(this.rgn_w / 3, 0);
				}.bind(this));

				$('zone').makeDraggable({
					container : 'navwin',
					// Take a note of the starting coords of our drag
					// zone
					onStart : function() {
			...
			refreshLoadBar : function() {
				...
				$('loadBar')
						.set('html', 'loading&nbsp;:&nbsp;' + Math.round(this.nTilesLoaded / this.nTilesToLoad * 100) + '%');
			 ...
			 */
			setScale : function() {
				var pixels = 1000 * this.scale * this.wid / this.max_width;
				// x1000 because we want per m
				var label = '1m';
				if (pixels > 1000) {
					pixels = pixels / 100;
					label = '1cm';
			...
			load : function() {
				new Request({
					...
					onComplete : function(transport) {
						var response = transport || alert("Image " + this.images[0].src + " not found from server " + this.server);
						var p = eval("(" + response + ")");
						// var tmp = p.levels;
			 ...
			 */
			reCenter : function() {
				center_x = (this.wid - this.rgn_w) / 2;
				center_y = (this.hei - this.rgn_h) / 2;
				if (center_x >= 0) this.rgn_x = center_x;
				if (center_y >= 0) this.rgn_y = center_y;
			},
