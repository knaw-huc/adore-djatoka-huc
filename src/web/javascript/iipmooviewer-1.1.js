...
var TargetDrag = new Class({
	...
	initialize : function() {
		...
		if (Browser.Engine.trident) this.handles.ondragstart = function() {
			return false;
		};
...
var IIP = new Class(
		{
			...
			/*
			 * Initialize some variables. The constructor takes 4 arguments: i) The id
			 * of the main div element in which to create the viewer window ii) A hash
			 * containting: image: the full image path (or array of paths) on the
			 * server (required) server: the iipsrv server full URL (defaults to
			 * "/fcgi-bin/iipsrv.fcgi") credit: image copyright or information
			 * (optional) zoom: the initial zoom level (optional - defaults to 1)
			 * render: tile rendering style - 'spiral' for a spiral from the centre or
				...
				for (var m = 0; m < ntiles; m++) {
					...
					if (djatoka_x < this.max_width && djatoka_y < this.max_height) {
						for (n = 0; n < this.images.length; n++) {
							...
							// djatoka mods
							var src = this.server + "?url_ver=Z39.88-2004&rft_id=" + this.images[n].src + "&svc_id=" + this.svc_id
							...
							// properly fired
							// var src =
							// this.server+"?FIF="+this.images[n].src+"&cnt="+this.contrast+"&sds="+this.images[n].sds+"&jtl="+this.res+","
							// + k;
							tile.set('src', src);
			...
			setOpenURL : function() {
				...
				this.openUrl = this.server + "?url_ver=Z39.88-2004&rft_id=" + this.images[0].src + "&svc_id=" + this.svc_id
						+ "&svc_val_fmt=" + this.svc_val_fmt + "&svc.format=image/jpeg&svc.level=" + this.res
						+ "&svc.rotate=0&svc.region=" + top_left_y + "," + top_left_x + "," + h + "," + w;
			},
			...
			createWindows : function() {
				// Get our window size - subtract some pixels to make sure the browser
				// never
				// adds scrollbars
			...
			// var response = transport || alert( "No response from server " +
			// this.server );
